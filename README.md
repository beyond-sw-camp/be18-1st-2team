...
### 1. 📋 선수 및 팀 관리 기능
선수, 포지션, 팀, 스태프 등록 및 정보 관리

계약 정보, 입단일, 신체 정보 등 선수 이력 데이터 관리

포지션 다중 등록 지원 (player_position)

트리거를 통한 스태프 및 감독 중복 입력 시 예외처리

### 2. ⚾ 경기 및 상세 기록 관리
game, game_record_detail을 통한 경기 정보 및 선수별 상세 기록 입력

경기별 투수·타자 기록 분리 관리 (이닝, 피안타, 타점 등)

### 3. 🔁 자동 집계 트리거 기능
트리거 기반 누적 통계 자동 계산

game_record_detail 입력 시 → player_record_detail 자동 갱신

player_record_detail 입력/수정 시 → player_record 자동 지표 계산 (AVG, OBP, SLG, OPS, ERA 등)

### 4. 📊 선수 성적 지표 분석
시즌별 타자/투수 통산 기록

타율, 출루율, 장타율, OPS, 평균자책점, WHIP, 수비율, 도루성공률 등 지표 제공

조건별 정렬/검색 쿼리 활용 가능

### 5. 🏆 팀 성적 관리 기능
경기 결과에 따른 팀별 시즌 성적(승/패/무, 승률) 자동 집계

team_record 테이블과 트리거로 승률 계산 자동화
...
<table>
  <tr>
    <td>김민준</td>
    <td>
     직접 db를 구축하고 프로젝트를 통해 다양한 경험을 할 수 있어 좋았습니다. <br>
     db를 생성하고 쿼리문을 작성하면서 초기 기획, 요구사항 분석 단계가 중요하다는 것을 알게되었고,
     팀원들과의 소통이 효율적인 시스템을 만드는데 큰 비중을 차지한다는 것도 알게되었습니다. <br>
     부족한 부분이 있을 때 도움을 준 팀원들에게 고마웠고, 이번 프로젝트를 통해 더욱 성장할 수 있어서 보람찼습니다.
    </td>
  </tr>
  <tr>
    <td>박종원</td>
       <td>
        야구 지표가 이렇게 복잡한 줄은 몰랐습니다.<br>
        데이터를 넣을 때마다 트리거가 정상 작동하는지 하나씩 확인해야 해서 힘들었습니다.<br>
        이번 프로젝트를 통해 프로젝트 진행시 개발 지식뿐만 아니라 해당 도메인의 이해가 굉장히 중요하다는걸 절실히 느꼈습니다.<br>
        앞으로는 프로젝트 진행시 개발뿐만 아니라 도메인 이해도 잘하는 개발자로 거듭나고 싶습니다.<br>
        그래도 오랜만에 팀프로젝트를 해서 보람 있었고 알찼습니다.🔥    
     </td>
  </tr>
  <tr>
    <td>박진우</td>
    <td>
     다음과 같이 2가지를 느꼇다. 
     1. 중앙 서버 운영의 필요성
     초기에 PC 1대를 공용 DB 서버로 지정하여 팀원들이 해당 서버를 통해 실시간으로 작업을 공유하고 테스트했다면 동기화 문제를 대폭 줄일 수 있었을 것.
     2. 버전 관리 체계 도입
     SQL 파일도 Git 등의 형상관리 도구를 통해 관리하고, Pull Request 기반으로 변경사항을 검토하는 과정을 도입했더라면 실수를 조기에 방지할 수 있었을 것.
    </td>
  </tr>
  <tr>
    <td>윤석현</td>
    <td>프로젝트를 진행함에 있어서 도메인 지식이 중요하다는 것을 배웠습니다. 평소 관심없던 분야의 프로젝트를 진행하려하니 방대한 리서치가 필요했고 진행 도중 팀원들과 도움을 주고 받으며 협업의 진정한 의미를 깨달을 수 있었습니다.</td>
  </tr>
  <tr>
    <td>최유경</td>
    <td>
     이번 프로젝트를 통해 직접 db를 구축하고 쿼리문을 통해 검증을 해보면서 헷갈렸던 부분들에 대해서 정리 할 수 있었고 github사용과 쿼리문 작성에 익숙해질 수 있었습니다. 
     직접 erd를 작성해보면서 데이터를 효율적으로 사용하기 위해서는 어떻게 저장하면 좋을지에 대해 깊게 고민해볼 수 있었고 또한,
     데이터에 대한 이해가 선행되어야 한다는 사실을 느낄 수 있는 기회가 된 것 같아 좋았습니다.<br> 
     초반에 기능과 데이터 크기에 대해서 명확히 구분짓고 넘어가지 않아 진행하면서 팀원들과 생각의 차이가 있기도 했습니다. 하지만 의견을 나누며 조율하고 수정해가며 협업을 몸소 배울 수 있었습니다. 
     또한, 저의 코드나 설계의 부족한 부분을 팀원 덕분에 알게되었고 팀원의 코드나 설계를 보며 더욱 공부해 볼 수 있었습니다.
    </td>
  </tr>
</table>
